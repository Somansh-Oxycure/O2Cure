...
const B2C_ENVIRONMENTS = [
  ...
  { value: "building", label: "Villa / Independent House" },
            ...
            {options.map((opt, i) => (
              <li
                ...
                className={cn(
                  ...
                  opt.value === value
                    ? "bg-[#F8FAFC] text-[#2563EB] font-medium"
...
export function CommercialEnquiryForm({
  ...
  const [formData, setFormData] = useState<EnquiryFormData>({
    name: "",
    company: "", // Not used in commercial but required by type
    email: "",
    phone: "",
    environment: "",
    message: "",
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    // Bypassing 'company' validation for commercial
    const dataToValidate = { ...formData, company: "Commercial Client" };
      ...
      <AnimatePresence mode="wait">
        {isSuccess ? (
          <FormSuccess
            key="success"
            heading="We're on it."
            message="An O2Cure Commercial Specialist will contact you within 24 hours."
          />
        ) : (
          <motion.form
            key="form"
          ...
          >
            {/* Form header */}
            <div className="mb-6">
              <h2 className="text-[1.3rem] font-bold text-[#0F172A] leading-tight tracking-[-0.02em]">
                Schedule Air Diagnostics
              </h2>
              <p className="mt-1.5 text-[0.85rem] text-gray-500 font-light leading-relaxed">
                Tell us about your workspace — we'll design the right purification solution.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 sm:gap-6">
              ...
              <div className="sm:col-span-2">
                <FloatingTextarea
                  ...
                  label="Tell us about your workspace & concerns"
                  optional
            ...
            <div className="pt-4">
              <motion.button
                type="submit"
                id="commercial-submit-btn"
                disabled={isSubmitting}
                className="group relative flex h-14 w-full items-center justify-center overflow-hidden rounded-full bg-[#2563EB] px-8 text-[0.9rem] font-semibold tracking-wide text-white transition-colors duration-300 hover:bg-[#1D4ED8] disabled:opacity-60"
